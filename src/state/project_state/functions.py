...
class ProjectStateFunctionsMixin:
    ...
    target_functions: dict[str, list[Callable[..., Any]]]
    imported_modules: dict[str, Any]
    reference_modules: list[str]

    def get_function(
        self: "ProjectStateLike", function_name: str
        ...
        for module_path in import_order:
            functions = self.target_functions.get(module_path, [])
        ...
        for functions in self.target_functions.values():
            for func_item in functions:
                if (
                    hasattr(func_item, "__name__")
                    and func_item.__name__ == function_name
    ...
    def find_reference_function(
        ...
        return self.find_reference_function_str(
            func_name,
    ...
    def find_reference_function_str(
        ...
        Find the reference implementation of a function.
        """
        return self._find_reference(func_name, target_module, src_module, src_qualname)
    ...
    def _find_reference(
        ...
        project_functions = self.target_functions
    ...
    def _module_distance(
        self: "ProjectStateLike", a: str, b: Optional[str]
    ) -> tuple[int, int]:
        if not b:
            return (10**9, 0)
        a_parts = a.split(".")
