...
def test_nom001_inherits_directly_from_compile_emits_nominal_type_rule() -> None:
    source = """
...
class CsvRepository(BaseRepository):
    __archtest__: Annotated[
        None,
        inherits_directly_from("source_module.BaseRepository"),
    ]
"""
    ...
    assert [rule.rule_id for rule in compiled_rules] == [
        "NOM001/inherits_directly_from/v1"
    ]
...
def test_nom001_inherits_directly_from_compile_non_class_emits_invalid_target_evidence() -> (
    None
):
    source = """
from typing import Annotated
...
"""
    ...
    assert rules[0].params["decorator"] == "inherits_directly_from"
    assert rules[0].params["param"] == "target_kind"
    assert compiler_results == []
    assert [item.type for item in evidence] == ["compiler_invalid_target"]


def test_nom001_inherits_directly_from_compile_empty_base_emits_compiler_evidence() -> (
    None
):
    source = """
from typing import Annotated
...
class CsvRepository:
    __archtest__: Annotated[None, inherits_directly_from("")]
"""
...
def test_nom001_inherits_directly_from_compile_unresolved_base_emits_compiler_evidence() -> (
    None
):
    source = """
from typing import Annotated
...
class CsvRepository:
    __archtest__: Annotated[
        None,
        inherits_directly_from("source_module.MissingBase"),
    ]
"""
...
def test_nom001_inherits_directly_from_compile_ambiguous_simple_name_emits_compiler_evidence() -> (
    None
):
    source_entities = [
        *_extract_entities(
...
class CsvRepository:
    __archtest__: Annotated[None, inherits_directly_from("BaseRepository")]
""",
...
def test_nom001_inherits_directly_from_invalid_target_sentinel_rule_evaluates_failed() -> None:
    source = """
from typing import Annotated
from pythonarchtesting.rules import inherits_directly_from

def build() -> None:
    __archtest__: Annotated[None, inherits_directly_from("source_module.BaseRepository")]
"""
    target = """
def build() -> None:
    return None
"""
    ...
    assert sentinel_rules[0].rule_id.startswith(
        "NOM001/inherits_directly_from/invalid_declaration/"
    )
